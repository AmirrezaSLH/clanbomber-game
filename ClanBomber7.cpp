...
Mix_Music *prize = NULL;
Mix_Music *dead = Mix_LoadMUS( "voice/Dead.wav" );
const int NP = 2;
SDL_Color textColor = { 0, 255, 0 }, bgColor = { 0, 0, 0 };
string s = "", s1 = "";
SDL_Event e;
int j = 0, model = 1, playernum = 0, Mapnum = 0;
int map[100][100];
string map2[100]; 	 
bool MapFlag = false;
bool finish = false;
...
void PlaySound ( Mix_Music *music, int t )
{
...
{
	prize = Mix_LoadMUS( "voice/Prize.wav" );
	for ( int i = 0; i < NP; i++ )
	{
...
void Exp ( int x, int y, int i, int j, int r, int n, int t )
{
	...
	while ( r1 > 0 )
	{
		for ( int p = 0; p < NP; p++ ) if ( player[p].x/40 == x && player[p].y/40 == y ) player[p].f = false;
		for ( int p = 0; p < 100; p++ )
		{
