...
void DrawAll ()
{
	...
	for ( int i = 0 ; i < 100 ; i++ )
		if ( prs[i].x > 0 && prs[i].y > 0 && ( prs[i].r < 3 || prs[i].r == 6 ) ) 
		{	
...
void Exp ( int x , int y , int i , int j , int r , int n , int t )
{
	...
	while ( r1 > 0 )
	{
		...
		if ( map[x][y] == -3 )
		{	
			for ( int k = 0 ; k < 100 ; k++ ) if ( prs[k].x == x && prs[k].y == y ) 
			{
				prs[k].x = -1 ;
				prs[k].y = -1 ;
			}
...
void DrawBox ( int x = 300 , int y = 450)
{
	boxRGBA( screen2, 0, 0, x, y, 0, 0, 200, 16 );
	SDL_Rect onset;
   	onset.x = 350 ;
   	onset.y = 200 ;
...
void menu ()
{
	...
	while ( e.type != SDL_QUIT )
	{
		...
		if ( e.type == SDL_MOUSEBUTTONDOWN ) 
		{
			...
			if (( x > 400 && x < 600 ) && ( y > 450 && y < 550 )) SDL_Quit () ;
		}
		SDL_Flip ( screen );
	}
}
